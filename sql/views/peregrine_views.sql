...
-- ============================================================================
-- Views for Peregrine prompt ingestion and library monitoring

-- 1. View: vw_peregrine_prompt_library
-- Unified Peregrine prompt library with source context
CREATE OR REPLACE VIEW vw_peregrine_prompt_library AS
SELECT 
...
WHERE npl.product_id = (SELECT id FROM products WHERE product_code = 'peregrine');

-- 2. View: vw_peregrine_stage4_candidates
-- Re-expose Stage 4 Cat-Astrophic prompt candidates (for Peregrine ingestion)
CREATE OR REPLACE VIEW vw_peregrine_stage4_candidates AS
SELECT 
    t.id AS cat_turn_id,
