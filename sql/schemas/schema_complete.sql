...
-- ============================================================================
-- Nexus uses completed, successful Stage 4 prompts generated by Cat-Astrophic
-- OR client-provided prompts submitted by tenants.

-- Table: client_prompt_submissions
-- Client-provided prompts for Nexus (manual or API-submitted)
...
-- Table: nexus_prompt_library
-- Unified Nexus prompt library containing Stage 4 Cat-Astrophic prompts + client submissions
CREATE TABLE nexus_prompt_library (
    ...
    CONSTRAINT fk_nexus_prompt_library_product FOREIGN KEY (product_id)
        REFERENCES products(id) ON DELETE RESTRICT,
...
CREATE INDEX idx_nexus_prompt_library_hash ON nexus_prompt_library(prompt_hash);
CREATE INDEX idx_nexus_prompt_library_gold ON nexus_prompt_library(gold);
CREATE INDEX idx_nexus_prompt_library_cat_turn ON nexus_prompt_library(cat_turn_id);

-- Table: product_prompt_lineage
-- Cross-product traceability between AI-Range prompts and Nexus library entries
...
CREATE INDEX idx_prompt_lineage_products ON product_prompt_lineage(ai_range_product_id, nexus_product_id);

-- Trigger: auto-link AI-Range Stage 4 prompts to Nexus library entries
CREATE OR REPLACE FUNCTION trg_link_nexus_prompt_lineage()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.source_type = 'cat-astrophic' AND NEW.cat_turn_id IS NOT NULL THEN
        ...
        ON CONFLICT (ai_range_turn_id, nexus_prompt_id) DO NOTHING;
    END IF;
...
CREATE TRIGGER trg_nexus_prompt_lineage
AFTER INSERT ON nexus_prompt_library
FOR EACH ROW
EXECUTE FUNCTION trg_link_nexus_prompt_lineage();
...
-- View: vw_nexus_stage4_prompt_candidates
-- Completed, successful Stage 4 Cat-Astrophic prompts eligible for Nexus
CREATE OR REPLACE VIEW vw_nexus_stage4_prompt_candidates AS
SELECT 
        t.id AS cat_turn_id,
...
-- Trace AI-Range prompts through Nexus library entries
...
SELECT 
        ...
        npl.prompt_text AS nexus_prompt_text,
        npl.status AS nexus_status,
        npl.created_at AS nexus_created_at,
...
JOIN conversations c ON t.conversation_id = c.id
JOIN nexus_prompt_library npl ON pll.nexus_prompt_id = npl.id
WHERE t.product_id = (SELECT id FROM products WHERE product_code = 'ai-range')
    AND npl.product_id = (SELECT id FROM products WHERE product_code = 'nexus');
...
CREATE INDEX idx_nexus_conversations_created ON nexus_alpha.conversations(created_at DESC);

CREATE INDEX idx_nexus_turns_conversation ON nexus_alpha.turns(conversation_id);
...
CREATE INDEX idx_risk_metrics_benchmarking ON nexus_alpha.risk_metrics(benchmarking);
...
CREATE INDEX idx_robustness_benchmarking ON nexus_alpha.robustness_analysis(benchmarking);
...
CREATE INDEX idx_fragility_benchmarking ON nexus_alpha.fragility_scores(benchmarking);
...
CREATE INDEX idx_sycophancy_analysis_benchmarking ON nexus_alpha.sycophancy_analysis(benchmarking);
